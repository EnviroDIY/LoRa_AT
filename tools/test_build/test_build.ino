...
// Test WiFi Functions
  ...
  // Test TCP functions
  modem.maintain();
  TinyLoRaClient client;
  TinyLoRaClient client2(modem);
  TinyLoRaClient client3(modem, 1);
  client.init(&modem);
  client.init(&modem, 1);
...
#if defined(TINY_LORA_HAS_SSL)
  // modem.addCertificate();  // not yet impemented
  // modem.deleteCertificate();  // not yet impemented
  TinyLoRaClientSecure client_secure(modem);
  TinyLoRaClientSecure client_secure2(modem);
  TinyLoRaClientSecure client_secure3(modem, 1);
...
#if defined(TINY_LORA_HAS_CALLING) && not defined(__AVR_ATmega32U4__)
  modem.callNumber(String("+380000000000"));
  modem.callHangup();

#if not defined(TINY_LORA_SEQUANS_MONARCH)
  modem.callAnswer();
...
#if defined(TINY_LORA_HAS_SMS) && not defined(__AVR_ATmega32U4__)
  modem.sendSMS(String("+380000000000"), String("Hello from "));

#if not defined(TINY_LORA_XBEE) && not defined(TINY_LORA_SARAR4)
  modem.sendUSSD("*111#");
#endif

#if not defined(TINY_LORA_XBEE) && not defined(TINY_LORA_M590) && \
    not defined(TINY_LORA_SARAR4)
...
#if defined(TINY_LORA_HAS_GSM_LOCATION) && not defined(__AVR_ATmega32U4__)
...
#if defined(TINY_LORA_HAS_GPS) && not defined(__AVR_ATmega32U4__)
  modem.enableGPS();
  modem.getGPSraw();
  float latitude  = -9999;
...
#if defined(TINY_LORA_HAS_NTP) && not defined(__AVR_ATmega32U4__)
...
#if defined(TINY_LORA_HAS_BATTERY)
...
#if defined(TINY_LORA_HAS_TEMPERATURE)
